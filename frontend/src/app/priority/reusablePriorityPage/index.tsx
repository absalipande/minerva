  ...
  return (
    <div className='m-5 p-4'>
      ...
      <Header
        name='Priority Page'
        buttonComponent={
          <button className='mr-3 rounded bg-zinc-700 px-4 py-2 font-bold text-white hover:bg-zinc-600' onClick={() => setIsModalNewTaskOpen(true)}>
            Add Task
          </button>
        }
      />
      <div className='mb-4 flex justify-start gap-2'>
        <button className={`px-4 py-2 ${view === 'list' ? 'bg-gray-300' : 'bg-white'} rounded`} onClick={() => setView('list')}>
          List
        </button>
        <button className={`px-4 py-2 ${view === 'table' ? 'bg-gray-300' : 'bg-white'} rounded`} onClick={() => setView('table')}>
          Table
        </button>
      </div>
        ...
        view === 'list' ? (
          <div className='grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4'>
            {filteredTasks.map((task: Task) => (
              <TaskCard key={task.id} task={task} className='h-64 w-64' />
            ))}
          </div>
        ) : (
          view === 'table' && (
